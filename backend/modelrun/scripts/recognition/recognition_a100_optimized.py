#!/usr/bin/env python3
"""
Enhanced Korean Handwriting Recognition with Aspect-Ratio-Preserving Padding - A100 80GB Optimized
"""
import torch
...
def main(args):
    wandb.init(project=args.wandb_project, name=f"rec-a100-{datetime.now().strftime('%Y%m%d_%H%M')}", config=args)
    config = wandb.config
    device = torch.device(config.device)
    
    print("=== Korean Handwriting Recognition (A100 80GB Optimized) ===")
    ...
    for epoch in range(config.epochs):
        ...
        if avg_cer < best_cer:
            ...
            torch.save({
                ...
                'best_cer': best_cer,
                'config': dict(config)
            }, save_path / "korean_recognition_a100_best.pth")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Enhanced Korean Handwriting Recognition - A100 80GB Optimized")
    ...
    parser.add_argument("--save_dir", type=str, default="saved_models/recognition/enhanced_a100")
    
    parser.add_argument("--epochs", type=int, default=100)
    parser.add_argument("--batch_size", type=int, default=96)  # 30 → 96 (A100 80GB 최적화)
    ...
    parser.add_argument("--wandb_project", type=str, default="Korean-Handwriting-Recognition-A100")
