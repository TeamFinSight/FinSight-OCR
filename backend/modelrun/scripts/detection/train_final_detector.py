...
# OpenCV 멀티스레딩 비활성화 (DataLoader와 충돌 방지)
cv2.setNumThreads(0)
...
class DBNetLoss(nn.Module):
    def __init__(self, alpha=1.0, beta=20.0, ohem_ratio=3.0):
        super().__init__()
...
class DBNetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for bbox in label_json.get('bbox', []):
            ...
            points = np.array([[x, y] for x, y in zip(x_coords, y_coords)], dtype=np.float32)

            if len(points) < 3: continue

            # 입력 폴리곤을 항상 4개의 꼭지점을 가진 최소 면적 사각형으로 표준화
            rect = cv2.minAreaRect(points)
            box = cv2.boxPoints(rect)

            if Polygon(box).area < 1: continue
            polygons.append(box)
...
def main():
    ...
    config = {
        "img_size": 960,
        "batch_size": 12,  # 8 → 28 (A100 80GB 최적화)
        "epochs": 50,
        "lr": 1e-4,
    ...
    # CPU 병목 현상 완화를 위해 num_workers=2, pin_memory=True 설정
